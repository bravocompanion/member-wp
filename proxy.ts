...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/admin/import')) return NextResponse.next({ request })
...
export const config = {
  matcher: ['/((?!_next/static|_next/image|favicon.ico|manifest.webmanifest|sw.js|icon.svg|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)'],
}
